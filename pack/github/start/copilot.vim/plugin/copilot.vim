...
function! s:MapTab() abort
  ...
  let tab_map = maparg('<C-right>', 'i', 0, 1)
  if empty(tab_map)
    imap <script><silent><nowait><expr> <C-right> copilot#Accept()
  elseif tab_map.rhs !~# 'copilot'
    ...
    if get(tab_map, 'noremap') || get(tab_map, 'script') || mapcheck('<Left>', 'i') || mapcheck('<Del>', 'i')
      exe 'imap <script><silent><nowait><expr> <C-right> copilot#Accept(' . tab_fallback . ')'
    else
      exe 'imap <silent><nowait><expr>         <C-right> copilot#Accept(' . tab_fallback . ')'
    endif
  endif
endfunction
